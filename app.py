...
# -----------------------------------------------------------------------------
# 1. APPLICATION CONSTANTS & CLIENT INITIALIZATION
# -----------------------------------------------------------------------------
# We grab the secret token directly from Streamlit's cloud encrypted vault framework
...
# -----------------------------------------------------------------------------
# 2. APPLICATION USER INTERFACE DESIGN (STREAMLIT FRONTEND)
# -----------------------------------------------------------------------------
...
# Create a clean data capture block for inputs
with st.container():
...
# Initialize persistent session states in Streamlit so data doesn't wipe when pages reload
...
# -----------------------------------------------------------------------------
# 3. PHASE 1: QUESTION GENERATION PIPELINE
# -----------------------------------------------------------------------------
if st.button("Generate Original Question"):
    if subject.strip() == "" or topic.strip() == "":
        ...
        st.write("Communicating with DeepSeek AI network... generating problem payload.")
        
        # Build the exact prompt structure tested inside Google Colab
        generation_prompt = f"""
        ...
        # Dispatch request to the deepseek-chat endpoint model
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[{"role": "user", "content": generation_prompt}],
            temperature=0.7  # Balanced temperature for organic question phrasing
        )
        
        # Save the answer to memory state
...
# -----------------------------------------------------------------------------
# 4. PHASE 2: STUDENT INTERACTION & AUTOMATED EVALUATION LOOP
# -----------------------------------------------------------------------------
if st.session_state.generated_question:
    ...
    if st.button("Submit Assessment Answer"):
        if student_answer.strip() == "":
            ...
            st.write("Evaluating submission metrics against database schemas...")
            
            # Construct the binary grading prompt logic
            grading_prompt = f"""
            ...
            EXPLANATION: [Provide a precise 1-sentence analytical review of the physics, mathematical, or computing rule violated or met.]
            """
            
            grading_call = client.chat.completions.create(
                model="deepseek-chat",
                messages=[{"role": "user", "content": grading_prompt}],
                temperature=0.1  # Highly deterministic setting for exact evaluation grading
            )
